...
public class EditorActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor> {
    ...
    private void savePet() {
        ...
        String mWeightOfPet = mWeightEditText.getText().toString().trim();

        // Check if this is supposed to be a new pet
        // and check if all the fields in the editor are blank
        if (mCurrentPetUri == null &&
                TextUtils.isEmpty(nameOfPet) && TextUtils.isEmpty(mBreedOfPet) &&
                TextUtils.isEmpty(mWeightOfPet) && mGender == PetEntry.GENDER_UNKNOWN) {
        ...
        int weight = 0;
        if (!TextUtils.isEmpty(mWeightOfPet)) {
            weight = Integer.parseInt(mWeightOfPet);
        }
